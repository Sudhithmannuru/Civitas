...
A multilingual web app ("Civitas") that tells refugees/immigrants which U.S. federal benefits they likely qualify for, with deadlines, required docs, and next steps. Stack: Next.js App Router, Supabase (auth + Postgres + Storage), OpenAI (`openai` SDK).
...
- `lib/openai.ts` and the service-role Supabase client are **server-only**. `next.config.ts` lists `openai` under `serverExternalPackages`. Never import these from client components or expose the API/service keys to the browser.
...
- Model constants live in `lib/openai.ts`: `GPT = "gpt-4o"`, `GPT_MINI = "gpt-4o-mini"`.
...
2. **Rule *application* is evaluated in code** (`lib/eligibility/engine.ts`) against `database/benefits.json` (25 programs, each with a structured `{all,any,not,var,is,lte,gte,fpl}` `rule`). Statuses are `likely_eligible | maybe_eligible | not_eligible | needs_human_review`.
3. OpenAI (`gpt-4o`) writes warm plain-language narratives (why / next steps / summary) **in the user's language**, then an independent verification pass audits those narratives against the source records.
4. Results are ranked (soonest deadline first) and saved to `eligibility_results` via the service client.

> Note: `database/README.md` states eligibility is "decided by rules, not AI." That matches the current implementation: derived inputs and rule application are deterministic; the model only generates and verifies display text.
...
- `getTranslations(code)` returns English directly, else reads the `ui_translations` Supabase cache, else asks OpenAI to translate the whole `en.json` (preserving keys and `{placeholders}`) and upserts the result for all future users. Add UI strings to `en.json`; other languages are generated/cached on demand.
